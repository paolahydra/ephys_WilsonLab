...
clear
flyNum     = 298; % use from fly 298 on (ONLY)
cellNum    = 1;
Nfullblocks2keep = [];
...
assert(length(unique(trialIndicesFullBlock)) == 87);
...
SPIKEDET_STDMULTIPL = 4;
delayBetweenSpikePeaksMs = 8; %this really also depends on the threshold...
...
for stim = 1:nTrialTypes
    ...
    for i = 1:sum(trialIndicesFullBlock==stim)
        ...
        try
            ...
            warning('skipped one prepost2 stim record')
        end
...
directions = metadata.dec.directions(4:end);
intensities = metadata.dec.intensities(4:end);

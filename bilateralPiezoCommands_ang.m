function [stimset] = bilateralPiezoCommands_ang(ang_displacements,N_withinPseudoBlock,N_nPseudoBlocks,factor_jointZero,singleStimDur, sample_fs, experimentHandle, armL, armR )
...
N_conditionsPerAntenna = length(ang_displacements);
...
i_vZero = find(ang_displacements==0);
...
for n = 1 : length(both)
    ...
    lDisplacements(n) = displacementsL(lAnt(n));
    rDisplacements(n) = displacementsR(rAnt(n));
end
...
stimset.dec.joint2L_decoder_displ = ang_displacements(L);
stimset.dec.joint2R_decoder_displ = ang_displacements(R);
stimset.dec.joint2L_decoder_i = L;
stimset.dec.joint2R_decoder_i = R;
stimset.dec.i2displ_decoder = ang_displacements;

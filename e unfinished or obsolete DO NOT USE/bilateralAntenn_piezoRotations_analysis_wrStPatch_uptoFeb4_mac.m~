...
clearvars -except voltage sensorLfull sensorRfull flyfolder timestamps cellNum flyNum basename metadata metadataname cellfolder Nfullblocks2keep
...
SPIKEDET_STDMULTIPL = 2.5;
delayBetweenSpikePeaksMs = 5; %this really also depends on the threshold...
...
%% % plot all traces with two preceding and two following stimuli, arranged vertically
figFolder = fullfile(flyfolder, sprintf('singleStimsTraces_%s_%s',basename, metadata.key.ID));
figure('WindowStyle', 'normal', 'Position', [1071           1         368         804])

Ntr2plot = Nfullblocks;

[ha, pos] = tight_subplot( Ntr2plot, 1, [0.025, 0.05]);
YLIM = [-50, 10];

ha_idx = 1;
for stim = 1:length(peristim)
    trNumbers = round(linspace(1, sum(trialIndicesFullBlock==stim), Ntr2plot));
    blNumbers = round(linspace(1, Nfullblocks, Ntr2plot));
    for i_tr = 1 : Ntr2plot
        trN = trNumbers(i_tr);
...
savefig(sprintf('sampleTrials_voltageTrace_preafter_%s_all_mac', metadata.key.ID))
export_fig(sprintf('sampleTrials_voltageTrace_preafter_%s_all_mac.pdf', metadata.key.ID))
...
save(fullfile(flyfolder, sprintf('data_%s_%s_mac.mat',metadata.experimentHandle.basename, metadata.key.ID)), ...

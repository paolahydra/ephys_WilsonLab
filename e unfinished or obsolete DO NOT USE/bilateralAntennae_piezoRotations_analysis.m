...
%% SAMPLE STIMULI
max_leftIpsi_n70deg_120amp = find(metadata.dec.directions(1:71) == -70 & metadata.dec.intensities(1:71) == 120);
max_rightContra_70deg_120amp = find(metadata.dec.directions(1:71) == 70 & metadata.dec.intensities(1:71) == 120);
max_Head_0deg_120amp = find(metadata.dec.directions(1:71) == 0 & metadata.dec.intensities(1:71) == 120);
...
%% average filtered Vm and scatter in 2d
for stim = 1:nTrialTypes
   meanfiltVm(stim) =  mean(mean(perist(stim).medFiltTrace));
   meanFR(stim) = mean(sum(perist(stim).spikes,2));
end
I30 = metadata.dec.intensities == 30;
I60 = metadata.dec.intensities == 60;
I120 = metadata.dec.intensities == 120;
f = figure; hold on
plot(metadata.dec.directions(I30), meanfiltVm(I30)-meanfiltVm(1)); 
plot(metadata.dec.directions(I60), meanfiltVm(I60)-meanfiltVm(1));
plot(metadata.dec.directions(I120), meanfiltVm(I120)-meanfiltVm(1));
legend('30', '60', '120')
legend boxoff
...
f = figure; hold on
plot(metadata.dec.directions(I30), meanFR(I30)-meanFR(1)); 
plot(metadata.dec.directions(I60), meanFR(I60)-meanFR(1));
plot(metadata.dec.directions(I120), meanFR(I120)-meanFR(1));
legend('30', '60', '120')
legend boxoff
legend('Location', 'NorthWest')
xlabel('estimated wind direction')
...
save(fullfile(metadata.experimentHandle.flyfolder, sprintf('data_%s_%s.mat',metadata.experimentHandle.basename, metadata.key.ID)), ...
    'meanfiltVm', 'meanFR', 'metadata', 'perist')

%%
aL = metadata.dec.angL(4:end);
aR = metadata.dec.angR(4:end);
mapc3 = (brewermap(125, 'RdYlGn'));

figure; hold on; axis image
xlabel(' Left antenna (ipsi) - angular displ')
ylabel(' Right antenna (contra) - angular displ')
scatter(aL, aR, 60, meanfiltVm, 'filled')
title('Vm')
colorbar
colormap(mapc3)
export_fig(sprintf('scatter_absVm_%s.pdf', metadata.key.ID))

figure; hold on; axis image
xlabel(' Left antenna (ipsi) - angular displ')
ylabel(' Right antenna (contra) - angular displ')
scatter(aL, aR, 60, meanfiltVm-meanfiltVm(1), 'filled')
title('change of Vm')
colorbar
colormap(mapc3)
export_fig(sprintf('scatter_relVm_%s.pdf', metadata.key.ID))


figure; hold on; axis image
xlabel(' Left antenna (ipsi) - angular displ')
ylabel(' Right antenna (contra) - angular displ')
scatter(aL, aR, 60, meanFR-meanFR(1), 'filled')
title('change of firing rate')
colorbar
colormap(mapc3)
export_fig(sprintf('scatter_relFR_%s.pdf', metadata.key.ID))

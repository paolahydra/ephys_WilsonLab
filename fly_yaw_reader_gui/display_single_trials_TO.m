function [] = display_single_trials_TO( task, trial_time, trial_data, viz_figs, total_duration, pre_stim_t, stim_t, experiment_dir )
...
elseif( strcmp(task, 'OdorNoWind') == 1 )
    cur_color = colors{4};
elseif( strcmp(task, 'OdorLeftWind') == 1 | strcmp(task, 'LeftWind') == 1 )
    cur_color = colors{1};
elseif( strcmp(task, 'OdorRightWind') == 1 | strcmp(task, 'RightWind') == 1)
    cur_color = colors{2};
elseif( strcmp(task, 'OdorCenterWind') == 1 | strcmp(task, 'CenterWind') == 1)
    cur_color = colors{3};
else
...
use_calibration = 1;
[ t, vel_forward, vel_side, vel_yaw ] = get_velocity_ephysrig_TO(trial_time, trial_data, experiment_dir, use_calibration );

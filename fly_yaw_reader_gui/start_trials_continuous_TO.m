function [ output_args ] = start_trials_continuous_TO( run_obj )
...
%     if run_obj.IsBehvaior % behavior only
%        settings.sampRate = 4000; % lower sampling rate for behavioral experiments
%        msgbox('Fs = 4000 Hz')
%     end    
    ...
    pre_out_chan_0 = zeros( task_cnt, SAMPLING_RATE * total_duration ); % AO0: 200B for current injection
    pre_out_chan_1 = zeros( task_cnt, SAMPLING_RATE * total_duration ); % AO1: speaker output
    ...
    % set the stimulation pulse for the valves
    ...
    %% setup sound (TO)
    Sound = SineWave;
    Sound.sampleRate = SAMPLING_RATE;
    Sound.carrierFreqHz = 200; % [Hz]
    Sound.sineDur = 2; % [s]
    Sound.startPadDur = 2; % [s]
    Sound.endPadDur = 2; % [s]
    Sound.maxVoltage = 0.1; % [V]
    ...
    CAMERA_FPS = 32.0;
    camera_triggers(1:round(SAMPLING_RATE/CAMERA_FPS):end) = 1;
...
%             pre_out_chan_1(i,:) = stim*5.0;
        ...
        elseif( strcmp(cur_task, 'OdorCenterWind') == 1 )
            ...
            pre_out_chan_3(i,:) = stim_LED; % right LED
            pre_out_chan_5(i,:) = stim_wind; % main valve
            pre_out_chan_7(i,:) = stim_pinch; % center valve
        elseif( strcmp(cur_task, 'LeftWind') == 1 )
            pre_out_chan_5(i,:) = stim_wind; % main valve
            pre_out_chan_8(i,:) = stim_pinch; % left valve
        elseif( strcmp(cur_task, 'RightWind') == 1 )
            pre_out_chan_5(i,:) = stim_wind; % main valve
            pre_out_chan_6(i,:) = stim_pinch; % right valve
        elseif( strcmp(cur_task, 'CenterWind') == 1 )
            pre_out_chan_5(i,:) = stim_wind; % main valve
            pre_out_chan_7(i,:) = stim_pinch; % center valve        
        elseif( strcmp(cur_task, 'LeftSound') == 1 )
            pre_out_chan_9(i,:) = Sound;
            % add signal for relay here
        elseif( strcmp(cur_task, 'RightSound') == 1 )
            pre_out_chan_9(i,:) = Sound;
        else
    ...
    if 1 % ploting the pulse sequence for debugging
        TrialNum = 1;
